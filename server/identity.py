...
async def authenticate(request, scope: Optional[str] = None) -> Optional[str]:
    ...
    Order: OAuth RS256 Bearer (validated locally via JWKS, scoped) -> legacy X-API-KEY.
    Both the website (Auth Code + PKCE) and the app (daemon device grant) send real
    OAuth tokens, so there is NO email/password bridge and NO per-request IdP round-trip
    (see ecosystem/AUTHENTICATION.md non-negotiables). A present-but-invalid Bearer must
    401 — it never falls through to the key path.
    """
    ...
    return await resolve_agent_key(key)        # legacy path (phone app during migration)
