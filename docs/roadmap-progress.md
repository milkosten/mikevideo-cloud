...
| **P3** | IN_PROGRESS | **Channels (@handle) + public profile.** Migration: `channels`(user_id, handle unique, display_name, avatar, bio) or reuse user; `GET /api/channels/{handle}` (public: their public videos + counts). Web: a channel page (avatar, name, subscriber count, video grid), channel name+avatar on cards linking to it. App: channel view. |
