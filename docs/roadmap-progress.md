...
|---|---|---|
| **P1** | PENDING | **Engagement primitives.** Migration: `views`(count via a `video_views` event or a counter col), `likes`(video_id,user_id unique), `watch_progress`(video_id,user_id,position_sec,updated). Endpoints: `POST /api/videos/{id}/view`, `POST/DELETE /api/videos/{id}/like`, `GET/PUT /api/videos/{id}/progress`; add `views/likes/liked/progress` to video detail+list. Web: show view count + age on cards, like button + count on watch, **resume** playback from saved position + a "Continue watching" row. App: like button, resume, report views/progress. |
